...
- (void)initUI {
    ...
    if(self.notification.isWarning && self.notification.data.isCameraData) {
        ...
        XXButton *btnDelete = [XXButton blueButtonWithPoint:CGPointMake(0,view.frame.size.height+view.frame.origin.y+5) resize:CGSizeMake(311,BLUE_BUTTON_DEFAULT_HEIGHT)];
        btnDelete.center = CGPointMake(self.view.center.x, btnDelete.center.y);
        [btnDelete setTitle: NSLocalizedString(@"delete", @"") forState:UIControlStateNormal];
        [btnDelete addTarget:self action:@selector(deleteBtnPressed:) forControlEvents:UIControlEventTouchUpInside];
        [self.view addSubview:btnDelete];
    } else if ([self.notification.type isEqualToString:@"CF"]) {
        XXButton *btnAgree = [XXButton blueButtonWithPoint:CGPointMake(5,view.frame.size.height+view.frame.origin.y+5) resize:CGSizeMake(100, BLUE_BUTTON_DEFAULT_HEIGHT)];
        ...
        XXButton *btnRefuse = [XXButton blueButtonWithPoint:CGPointMake(0, 0) resize:CGSizeMake(100, BLUE_BUTTON_DEFAULT_HEIGHT)];
        ...
        XXButton *btnDelete = [XXButton blueButtonWithPoint:CGPointMake(0, 0) resize:CGSizeMake(100, BLUE_BUTTON_DEFAULT_HEIGHT)];
